(function ($) {
    ...
    $.expandTree = function (parentId, remoteLoading) {
        if (!parentId) {
            $.post("/Admin/Menu/List", { parentId: parentId }, function (result) {
                $("#tree-menu-item").html(result);
            });
        } else {
            ...
            if (remoteLoading) {
                $.post("/Admin/Menu/ExpandTreeItem", { parentId: parentId }, function (result) {
                    $(remoteContent).empty();
    ...
    _treeNodeChangePosition = function (sourceNode, targetNode) {
        $.post("/Admin/Menu/TreeNodeChangePosition", { sourceId: $(sourceNode).data("id"), targetId: $(targetNode).data("id") }, function (result) {
            var parentId = $(sourceNode).parents(".tree-node").data("id");
            if (!parentId) {
                $.post(result.AfterAction, function (partial) {
                    $("#" + result.TargetElementId).html(partial);
                });
            } else {
...
(function ($) {
    ...
    _loadDropDownMenu = function (parentId, remoteTarget, remote) {
        ...
        $.post("/Menu/CollapseMenu", { parentId: parentId }, function (result) {
